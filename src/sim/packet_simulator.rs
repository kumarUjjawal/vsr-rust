...
impl<P: Clone + Send + 'static> PacketSimulator<P> {
    ...
    pub fn submit_packet(&mut self, packet: P, path: Path) {
        ...
        let data = Data {
            expiry: self.ticks + one_way_delay,
            packet,
            path,
        };
        ...
        self.paths[path_index].push(data);
    }
    ...
    pub fn tick(&mut self) {
        ...
        for source in 0..self.options.node_count {
            for target in 0..self.options.node_count {
                let path_index =
                    (source as usize * self.options.node_count as usize) + target as usize;

                while let Some(data) = self.paths[path_index].peek() {
                    if data.expiry > self.ticks {
                        break;
                    }
                    ...
                    {
                        self.submit_packet(data.packet.clone(), data.path);
    ...
    fn partition_network(&mut self) {
        ...
        match self.options.partition_mode {
            ...
            PartitionMode::UniformSize => {
                ...
                let mut replicas: Vec<usize> = (0..replica_count).collect();
                replicas.shuffle(&mut self.prng);
                for (i, &replica_idx) in replicas.iter().enumerate() {
            ...
            PartitionMode::UniformPartition => {
                loop {
                    for i in 0..replica_count {
                        self.partition[i] = self.prng.random();
                    }
                    // Ensure the partition isn't trivial (all nodes in one group).
